...
namespace ImGuiVRHelperPluginAPI
{
	...
	/// Revision 006. Adds a per-client physical surface and authoritative pointer
	/// UV for mods that already own a 3D menu and raycast. Obtain via
	/// GetImGuiVRHelperInterface006(); nullptr against a helper older than 006.
	struct IImGuiVRHelperInterface006 : IImGuiVRHelperInterface005
	{
		/// Replace this client's physical surface state. This is a heartbeat: call
		/// while the surface is active. Passing nullptr clears the override and
		/// returns the client to the helper's normal HMD/controller placement and
		/// wand raycast. Unknown ids are ignored.
		virtual void SubmitClientSurface(uint32_t client_id, const ClientSurfaceState* state) = 0;
	};

	IImGuiVRHelperInterface006* GetImGuiVRHelperInterface006();

	/// Revision 007. Lets a client host its ImGui panel texture on geometry in
	/// the game's own scene graph. This avoids a second post-composited quad and
	/// therefore inherits the exact stereo camera and final skeleton transform
	/// used by that geometry. Obtain via GetImGuiVRHelperInterface007(); nullptr
	/// against a helper older than 007.
	struct IImGuiVRHelperInterface007 : IImGuiVRHelperInterface006
	{
		/// Acquire strong COM references to the client's panel texture and SRV.
		/// The handle must be released with ReleasePanelTexture, including when
		/// replacing it after a helper-side texture resize.
		virtual bool AcquirePanelTexture(uint32_t client_id, PanelTextureHandle* out) = 0;

		/// Releases both strong references in a handle and clears its fields.
		virtual void ReleasePanelTexture(PanelTextureHandle* handle) = 0;

		/// Suppress the helper's normal eye-submit panel/cursor composite for this
		/// client. Rendering callbacks, focus, input and API006 pointer UV remain
		/// active. Call with false before removing the client-owned geometry.
		virtual void SetExternalPanelHost(uint32_t client_id, bool enabled) = 0;
	};

	IImGuiVRHelperInterface007* GetImGuiVRHelperInterface007();

	/// Revision 008. Adds a global physical host that can display whichever
	/// focusable ImGui client is currently active. The content client keeps
	/// ownership of rendering and focus; the host supplies only geometry,
	/// pointer UV and the scene-graph surface. Obtain via
	/// GetImGuiVRHelperInterface008(); nullptr against helpers older than 008.
	struct IImGuiVRHelperInterface008 : IImGuiVRHelperInterface007
	{
		/// Publish the physical surface used by the currently focused client.
		/// This is a heartbeat while the host is active. Passing nullptr clears
		/// the host and restores each client's normal helper placement.
		virtual void SubmitPhysicalHostSurface(
			uint32_t host_client_id, const ClientSurfaceState* state) = 0;

		/// Acquire strong COM references for the currently focused client's
		/// panel. `out_content_client_id` identifies whose texture was acquired.
		/// Release the returned handle with ReleasePanelTexture.
		virtual bool AcquireFocusedPanelTexture(
			uint32_t host_client_id, PanelTextureHandle* out,
			uint32_t* out_content_client_id) = 0;

		/// Close the panel currently displayed by this physical host.
		virtual void ReleaseHostedPanel(uint32_t host_client_id) = 0;

		/// Move focus through the helper's ordered list of focusable panels.
		virtual void CycleHostedPanel(uint32_t host_client_id, int32_t direction) = 0;
	};

	IImGuiVRHelperInterface008* GetImGuiVRHelperInterface008();

	/// Revision 009. Adds generic discovery and direct selection for physical
	/// hosts that provide tabs, launchers or other client navigation UI.
	struct IImGuiVRHelperInterface009 : IImGuiVRHelperInterface008
	{
		/// Returns the number of focusable panels. If `out` is non-null, writes
		/// up to `capacity` entries in stable overlay order. Call with nullptr/0
		/// to query the required count. The host itself is included first.
		virtual std::size_t GetHostedPanels(
			uint32_t host_client_id, HostedPanelInfo* out, std::size_t capacity) = 0;

		/// Select a panel returned by GetHostedPanels. Returns false for stale,
		/// unknown or non-focusable ids. The selected client remains responsible
		/// for rendering its own ImGui content.
		virtual bool SelectHostedPanel(
			uint32_t host_client_id, uint32_t content_client_id) = 0;

		/// Let native chrome owned by the physical host receive controller
		/// buttons even when it overlaps the ImGui surface. This is a heartbeat;
		/// pass false as soon as the native control is no longer hovered.
		virtual void SetPhysicalHostInputPassthrough(
			uint32_t host_client_id, bool enabled) = 0;
	};

	IImGuiVRHelperInterface009* GetImGuiVRHelperInterface009();

	/// Revision 010. Lets a physical host request the exact pixel dimensions of
	/// the focused panel it displays, without changing the helper's resolution
	/// policy for clients rendered anywhere else.
	struct IImGuiVRHelperInterface010 : IImGuiVRHelperInterface009
	{
		/// Set the focused texture size while this physical host is active. The
		/// helper reallocates the focused client's panel when necessary. Pass 0,0
		/// to clear the request and return clients to normal helper sizing.
		virtual bool SetPhysicalHostPanelSize(
			uint32_t host_client_id, uint32_t width, uint32_t height) = 0;
	};
