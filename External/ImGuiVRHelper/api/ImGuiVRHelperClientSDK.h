...
namespace ImGuiVRHelperPluginAPI
{
	...
	public:
		...
		bool Connect(const char* name, const char* version, uint32_t flags)
		{
			...
			m_helper007 = GetImGuiVRHelperInterface007();
			m_helper006 = m_helper007 ? static_cast<IImGuiVRHelperInterface006*>(m_helper007) :
			                            GetImGuiVRHelperInterface006();
			return true;
		}
		...
		[[nodiscard]] bool SupportsClientSurface() const { return m_helper006 != nullptr; }
		[[nodiscard]] bool SupportsExternalPanelHost() const { return m_helper007 != nullptr; }

		/// Publish a client-owned physical surface and authoritative pointer UV.
		/// Passing nullptr clears the override. No-op against helpers older than 006.
		void SubmitClientSurface(const ImGuiVRHelperPluginAPI::ClientSurfaceState* state)
		{
			if (m_helper006 && m_id)
				m_helper006->SubmitClientSurface(m_id, state);
		}

		bool AcquirePanelTexture(ImGuiVRHelperPluginAPI::PanelTextureHandle* out)
		{
			return m_helper007 && m_id && m_helper007->AcquirePanelTexture(m_id, out);
		}

		void ReleasePanelTexture(ImGuiVRHelperPluginAPI::PanelTextureHandle* handle)
		{
			if (m_helper007)
				m_helper007->ReleasePanelTexture(handle);
		}

		void SetExternalPanelHost(bool enabled)
		{
			if (m_helper007 && m_id)
				m_helper007->SetExternalPanelHost(m_id, enabled);
		}
		...
		/// to the (possibly supersampled) panel, re-honors window
	...
	private:
		...
		IImGuiVRHelperInterface006* m_helper006 = nullptr;  // null if helper predates 006 (no client-owned surface)
		IImGuiVRHelperInterface007* m_helper007 = nullptr;  // null if helper predates 007 (no scene-graph host)
