...
namespace ImGuiVRHelperPluginAPI
{
	...
	/// Strong references to the texture backing a client's panel. Returned by
	/// Interface007::AcquirePanelTexture for clients that render the panel on
	/// geometry owned by their own scene graph. Both COM pointers carry one
	/// reference and must be returned through ReleasePanelTexture.
	struct PanelTextureHandle
	{
		uint32_t width;
		uint32_t height;
	...
	/// POD description of a focusable panel exposed to a physical host. Fixed
	/// buffers keep the public boundary independent of the helper's STL/CRT.
	struct HostedPanelInfo
	{
		uint32_t client_id;
	...
	enum HostedPanelFlags : uint32_t
	{
		kHostedPanelFlag_None = 0,
		kHostedPanelFlag_Focused = 1u << 0,
		kHostedPanelFlag_HostClient = 1u << 1,
		kHostedPanelFlag_HelperClient = 1u << 2,
	};
	...
	/// A client-owned interactive panel surface in Skyrim world space. This is
	/// intended for physical UI hosts (wrist menus, books, boards) that already
	/// have an authoritative scene-graph transform and raycast. `right` and `up`
	/// are unit vectors in Skyrim world axes; width/height are Skyrim game units.
	/// The helper derives the normal as right x up and converts the surface to VR
	/// tracking space in the same eye pass that renders it.
	struct ClientSurfaceState
	{
		...
		float pointer_u = 0.0f;
		float pointer_v = 0.0f;
	};

	enum ClientSurfaceFlags : uint32_t
	{
		kClientSurfaceFlag_None = 0,
		kClientSurfaceFlag_Visible = 1u << 0,
		kClientSurfaceFlag_PointerInPanel = 1u << 1,
	};
