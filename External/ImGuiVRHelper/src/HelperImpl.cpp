...
namespace
{
	...
	// Registration name of the helper's own settings/toast client.
...
namespace ImGuiVRHelper
{
	...
	void HelperImpl::UnregisterClient(uint32_t client_id)
	{
		...
		if (m_physical_host_client == client_id) {
			m_physical_host_client = 0;
			m_has_physical_host_surface = false;
			m_physical_host_input_passthrough = false;
			m_physical_host_surface = {};
		}
		if (m_physical_host_size_client == client_id) {
			m_physical_host_size_client = 0;
			m_physical_host_panel_width = 0;
			m_physical_host_panel_height = 0;
		}
	}

	void HelperImpl::PanelPixelSize(const ClientRecord& rec, unsigned int& width, unsigned int& height) const
	{
		if (m_physical_host_size_client != 0 && rec.clientId == m_focused_client &&
			m_physical_host_panel_width != 0 && m_physical_host_panel_height != 0) {
			width = m_physical_host_panel_width;
			height = m_physical_host_panel_height;
			return;
		}
	...
	bool HelperImpl::EnsureClientTextureLocked(ClientRecord& rec)
	{
		...
		if (rec.texture && rec.rtv && rec.allocatedWidth == pw && rec.allocatedHeight == ph)
			return true;
		if (!Globals::IsReady()) {
		...
		rec.allocatedWidth = pw;
		rec.allocatedHeight = ph;
		...
		ImGuiVRHelperPluginAPI::PanelHandle* out)
	{
		...
		// Report the real allocated panel size.
		unsigned int pw = 0, ph = 0;
	...
	void HelperImpl::SubmitClientSurface(uint32_t client_id,
		const ImGuiVRHelperPluginAPI::ClientSurfaceState* state)
	{
		std::scoped_lock lk{ m_mutex };
		auto it = m_clients.find(client_id);
		if (it == m_clients.end())
		...
		if (!state) {
			it->second.hasClientSurface = false;
			it->second.clientSurface = {};
			return;
		}

		const auto finite3 = [](const float v[3]) {
			return std::isfinite(v[0]) && std::isfinite(v[1]) && std::isfinite(v[2]);
		};
		if (!finite3(state->center) || !finite3(state->right) || !finite3(state->up) ||
			!std::isfinite(state->width_units) || !std::isfinite(state->height_units) ||
			!std::isfinite(state->pointer_u) || !std::isfinite(state->pointer_v) ||
			state->width_units <= 1e-4f || state->height_units <= 1e-4f) {
			it->second.hasClientSurface = false;
			return;
		}

		it->second.clientSurface = *state;
		it->second.clientSurface.pointer_u = std::clamp(state->pointer_u, 0.0f, 1.0f);
		it->second.clientSurface.pointer_v = std::clamp(state->pointer_v, 0.0f, 1.0f);
		it->second.hasClientSurface = true;
	}

	bool HelperImpl::AcquirePanelTexture(uint32_t client_id,
		ImGuiVRHelperPluginAPI::PanelTextureHandle* out)
	{
		if (!out)
			return false;
		*out = {};
		...
		std::scoped_lock lk{ m_mutex };
		auto it = m_clients.find(client_id);
		if (it == m_clients.end() || !EnsureClientTextureLocked(it->second))
			return false;

		unsigned int width = 0;
		unsigned int height = 0;
		PanelPixelSize(it->second, width, height);
		out->width = width;
		out->height = height;
		out->texture = it->second.texture.get();
	...
	void HelperImpl::ReleasePanelTexture(ImGuiVRHelperPluginAPI::PanelTextureHandle* handle)
	{
		if (!handle)
			return;
		if (handle->srv)
	...
	void HelperImpl::SetExternalPanelHost(uint32_t client_id, bool enabled)
	{
		std::scoped_lock lk{ m_mutex };
		auto it = m_clients.find(client_id);
		if (it != m_clients.end())
			it->second.externallyHosted = enabled;
	}

	void HelperImpl::SubmitPhysicalHostSurface(uint32_t host_client_id,
		const ImGuiVRHelperPluginAPI::ClientSurfaceState* state)
	{
		std::scoped_lock lk{ m_mutex };
		if (!m_clients.contains(host_client_id))
			return;

		if (!state) {
			if (m_physical_host_client == host_client_id) {
				m_physical_host_client = 0;
				m_has_physical_host_surface = false;
				m_physical_host_input_passthrough = false;
				m_physical_host_surface = {};
		...
		m_physical_host_client = host_client_id;
		m_physical_host_surface = *state;
		m_physical_host_surface.pointer_u = std::clamp(state->pointer_u, 0.0f, 1.0f);
		m_physical_host_surface.pointer_v = std::clamp(state->pointer_v, 0.0f, 1.0f);
		m_has_physical_host_surface = true;
	}

	bool HelperImpl::AcquireFocusedPanelTexture(uint32_t host_client_id,
		ImGuiVRHelperPluginAPI::PanelTextureHandle* out, uint32_t* out_content_client_id)
	{
		if (!out)
			return false;
		*out = {};
		if (out_content_client_id)
			*out_content_client_id = 0;

		std::scoped_lock lk{ m_mutex };
		// Texture acquisition is the host's readiness probe. Allow it before
		// SubmitPhysicalHostSurface so the helper's normal composite remains the
		// fallback until the scene geometry has actually accepted the texture.
		if (!m_clients.contains(host_client_id))
			return false;
		auto it = m_clients.find(m_focused_client);
		if (it == m_clients.end() || !EnsureClientTextureLocked(it->second))
			return false;

		PanelPixelSize(it->second, out->width, out->height);
		out->texture = it->second.texture.get();
		out->srv = it->second.srv.get();
		out->texture->AddRef();
		out->srv->AddRef();
		if (out_content_client_id)
			*out_content_client_id = m_focused_client;
		return true;
	}

	void HelperImpl::ReleaseHostedPanel(uint32_t host_client_id)
	{
		uint32_t focused = 0;
		uint32_t self = 0;
		{
			std::scoped_lock lk{ m_mutex };
			if (!m_has_physical_host_surface || m_physical_host_client != host_client_id)
				return;
			focused = m_focused_client;
			self = m_self_client_id;
		}
		if (focused == 0)
			return;
		if (focused == self)
			SettingsUI::SetVisible(false);
		ReleaseFocus(focused);
	}

	void HelperImpl::CycleHostedPanel(uint32_t host_client_id, int32_t direction)
	{
		{
			std::scoped_lock lk{ m_mutex };
			if (!m_has_physical_host_surface || m_physical_host_client != host_client_id)
		...
		CycleOverlay(direction < 0 ? -1 : 1);
	}

	std::size_t HelperImpl::GetHostedPanels(uint32_t host_client_id,
		ImGuiVRHelperPluginAPI::HostedPanelInfo* out, std::size_t capacity)
	{
		namespace API = ImGuiVRHelperPluginAPI;
		const auto overlayOrder = BuildOverlayOrder();
		std::vector<uint32_t> ids;
		ids.reserve(overlayOrder.size() + 1);
		ids.push_back(host_client_id);
		for (const auto& [id, name] : overlayOrder) {
			(void)name;
		...
		for (const uint32_t id : ids) {
			...
			if (out && count < capacity) {
				...
				info = {};
				info.client_id = id;
				info.client_flags = it->second.flags;
				if (id == m_focused_client)
					info.host_flags |= API::kHostedPanelFlag_Focused;
				if (id == host_client_id)
					info.host_flags |= API::kHostedPanelFlag_HostClient;
				if (id == m_self_client_id)
					info.host_flags |= API::kHostedPanelFlag_HelperClient;

				const auto copyText = []<std::size_t N>(char (&dest)[N], const std::string& source) {
					const std::size_t length = std::min(source.size(), N - 1);
					std::memcpy(dest, source.data(), length);
					dest[length] = '\0';
				};
				copyText(info.name, it->second.name);
				copyText(info.version, it->second.version);
			}
			++count;
		}
		return count;
	...
	bool HelperImpl::SelectHostedPanel(uint32_t host_client_id, uint32_t content_client_id)
	{
		uint32_t selfClient = 0;
		{
			std::scoped_lock lk{ m_mutex };
			if (!m_clients.contains(host_client_id))
				return false;
			const auto target = m_clients.find(content_client_id);
			if (target == m_clients.end())
				return false;

			constexpr uint32_t kNotHostSelectable =
			...
			const bool allowed = content_client_id == host_client_id ||
				content_client_id == m_self_client_id ||
				(target->second.flags & kNotHostSelectable) == 0;
			if (!allowed)
				return false;
			selfClient = m_self_client_id;
		}

		SettingsUI::SetVisible(content_client_id == selfClient);
		RequestFocus(content_client_id);
		logs::info("Physical host {} selected client {}", host_client_id, content_client_id);
		return true;
	}

	void HelperImpl::SetPhysicalHostInputPassthrough(uint32_t host_client_id, bool enabled)
	{
		std::scoped_lock lk{ m_mutex };
		if (m_has_physical_host_surface && m_physical_host_client == host_client_id)
			m_physical_host_input_passthrough = enabled;
	}

	bool HelperImpl::SetPhysicalHostPanelSize(
		uint32_t host_client_id, uint32_t width, uint32_t height)
	{
		std::scoped_lock lk{ m_mutex };
		if (!m_clients.contains(host_client_id))
			return false;
		if (width == 0 && height == 0) {
			if (m_physical_host_size_client == host_client_id) {
				logs::info("Physical host {} cleared its panel-size request", host_client_id);
				m_physical_host_size_client = 0;
				m_physical_host_panel_width = 0;
				m_physical_host_panel_height = 0;
			}
		...
		if (width < 320 || height < 240 || width > 7680 || height > 4320)
			return false;
		m_physical_host_size_client = host_client_id;
		m_physical_host_panel_width = width;
		m_physical_host_panel_height = height;
		logs::info("Physical host {} requested focused panel size {}x{}",
			host_client_id, width, height);
		return true;
	}

	bool HelperImpl::GetFocusedClientSurface(
		ImGuiVRHelperPluginAPI::ClientSurfaceState& out)
	{
		std::scoped_lock lk{ m_mutex };
		if (m_has_physical_host_surface &&
			m_clients.contains(m_physical_host_client) &&
			m_clients.contains(m_focused_client) &&
			(m_physical_host_surface.flags & ImGuiVRHelperPluginAPI::kClientSurfaceFlag_Visible) != 0) {
			out = m_physical_host_surface;
			if (m_physical_host_input_passthrough) {
				out.flags &= ~ImGuiVRHelperPluginAPI::kClientSurfaceFlag_PointerInPanel;
			}
			return true;
		}
		const auto it = m_clients.find(m_focused_client);
		if (it == m_clients.end() || !it->second.hasClientSurface ||
			(it->second.clientSurface.flags & ImGuiVRHelperPluginAPI::kClientSurfaceFlag_Visible) == 0)
			return false;
		out = it->second.clientSurface;
		return true;
	}

	bool HelperImpl::IsClientExternallyHosted(uint32_t client_id)
	{
		std::scoped_lock lk{ m_mutex };
		const auto it = m_clients.find(client_id);
		if (it == m_clients.end())
			return false;
		const bool focusedPhysicalHost = m_has_physical_host_surface &&
			m_clients.contains(m_physical_host_client) &&
			m_focused_client == client_id &&
			(m_physical_host_surface.flags & ImGuiVRHelperPluginAPI::kClientSurfaceFlag_Visible) != 0;
		return it->second.externallyHosted || focusedPhysicalHost;
	}
	...
	bool HelperImpl::ShouldSwallowInput() const
	{
		...
		// A physical host owns the surrounding native UI. Its buttons must see
		// controller events whenever the ray is outside the ImGui surface, and
		// may explicitly request passthrough for native chrome that overlaps it.
		if (ComboRecording::IsActive()) {
			return true;
		}
		...
		{
			std::scoped_lock lk{ m_mutex };
			if (m_has_physical_host_surface && m_clients.contains(m_physical_host_client)) {
				if (m_physical_host_input_passthrough)
					return false;
				return (m_physical_host_surface.flags &
					ImGuiVRHelperPluginAPI::kClientSurfaceFlag_PointerInPanel) != 0;
			}
			focusedClient = m_focused_client;
			const auto focused = m_clients.find(focusedClient);
			if (focused != m_clients.end())
				focusedFlags = focused->second.flags;
		}
		...
		if ((focusedFlags & ImGuiVRHelperPluginAPI::kClientFlag_PointerFocus) != 0) {
			auto& state = Overlay::State::GetSingleton();
			if (state.debugPointer.active.load(std::memory_order_relaxed)) {
				return true;
			}
	...
	bool HelperImpl::IsLiveToolFocused() const
	{
		...
		// A physical host owns the interaction model while it is active. If the
		// host opted into LiveTool, preserve locomotion for every content client
		// displayed on that surface; individual mods do not need an API update.
		if (m_has_physical_host_surface) {
			const auto host = m_clients.find(m_physical_host_client);
			if (host != m_clients.end() &&
				(host->second.flags & ImGuiVRHelperPluginAPI::kClientFlag_LiveTool) != 0) {
				return true;
			}
		}
		const auto it = m_clients.find(m_focused_client);
	...
	void HelperImpl::UpdateWandPointer()
	{
		...
		if (idx != vr::k_unTrackedDeviceIndexInvalid) {
			// controllerIndex identifies the hand that owns this pointer even when
			// the helper's built-in plane is missed and an external physical host
			// supplies the authoritative UV later in the frame.
			overlayState.wandState.controllerIndex = idx;
			ImVec2 uv;
	...
	uint32_t HelperImpl::DispatchToClients(const ImGuiVRHelperPluginAPI::Frame& baseFrame, float dt)
	{
		...
		auto& overlayState = Overlay::State::GetSingleton();
		// Focus may have changed since the early-frame input pass. Re-apply the
		// published physical surface before callbacks so GetPointer(), Frame flags,
		// and the cursor baked after rendering all observe the same hit.
		ApplyFocusedClientSurfacePointer();

		const bool wandHit = overlayState.wandState.isIntersecting ||
		                     overlayState.debugPointer.active.load(std::memory_order_relaxed);
		...
		for (const auto& sn : snapshot) {
			...
			// A physical host has one authoritative pointing controller. Older
			// clients commonly merge left/right button and stick state in their
			// embedded SDK, which lets the board-holding hand click or scroll the
			// hosted menu and can also hide a pointer-hand release behind the same
			// button held on the other hand. While the ray is on the physical panel,
			// expose input only from the controller that produced the pointer. Keep
			// pose/identity/haptic data intact, and keep both hands available off-panel
			// so client-defined VR shortcuts continue to work.
			if (sn.id == focused && wandHit &&
				overlayState.wandState.externalSurfaceActive.load(std::memory_order_relaxed)) {
				const auto pointerIndex = overlayState.wandState.controllerIndex;
				if (pointerIndex != vr::k_unTrackedDeviceIndexInvalid) {
					const uint32_t pointerToken = static_cast<uint32_t>(pointerIndex) + 1u;
					const bool leftPoints = perClient.left.haptic_token == pointerToken;
					const bool rightPoints = perClient.right.haptic_token == pointerToken;
					const auto clearInput = [](ImGuiVRHelperPluginAPI::Hand& hand) {
						hand.buttons_held = 0;
						hand.buttons_pressed = 0;
						hand.buttons_released = 0;
						hand.buttons_touched = 0;
						hand.trigger = 0.0f;
						hand.grip = 0.0f;
						hand.stick_x = hand.stick_y = 0.0f;
						hand.pad_x = hand.pad_y = 0.0f;
					};
					if (leftPoints != rightPoints) {
						clearInput(leftPoints ? perClient.right : perClient.left);
					}
				}
			}
	...
	void HelperImpl::ApplyFocusedClientSurfacePointer()
	{
		auto& wand = Overlay::State::GetSingleton().wandState;
		ImGuiVRHelperPluginAPI::ClientSurfaceState surface{};
		const bool hasSurface = GetFocusedClientSurface(surface);
		wand.externalSurfaceActive.store(hasSurface, std::memory_order_relaxed);
		if (!hasSurface)
			return;

		const bool hit = (surface.flags &
			ImGuiVRHelperPluginAPI::kClientSurfaceFlag_PointerInPanel) != 0;
		wand.isIntersecting.store(hit, std::memory_order_relaxed);
		wand.uvCoordinatesX.store(surface.pointer_u, std::memory_order_relaxed);
		wand.uvCoordinatesY.store(surface.pointer_v, std::memory_order_relaxed);
	}
	...
	void HelperImpl::PostProcessFrame(uint32_t focused, float dt)
	{
		...
		if (focused != 0) {
			if (auto tex = GetClientPanelTexture(focused)) {
				// A scene-graph host samples the client's base texture directly;
				// there is no eye-submit cursor pass above it. Bake the helper-owned
				// cursor into that texture after the client has rendered this frame.
				// The client clears/repaints the target every callback, so the cursor
				// cannot leave trails when its UV changes or exits the panel.
				if (IsClientExternallyHosted(focused) &&
					!(GetClientFlags(focused) & ImGuiVRHelperPluginAPI::kClientFlag_OwnCursor)) {
					InSceneOverlay::RenderCursorIntoPanel(tex.get());
				}
	...
	void HelperImpl::DispatchFrame(float dt)
	{
		...
		UpdateWandPointer();
		// A scene-graph host owns the real panel pose and raycast. Publish that hit
		// before drag/focus/input processing instead of briefly running those
		// systems against the helper's unrelated built-in overlay plane.
		ApplyFocusedClientSurfacePointer();
