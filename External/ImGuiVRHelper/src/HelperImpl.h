...
{
	class HelperImpl final : public ImGuiVRHelperPluginAPI::IImGuiVRHelperInterface010
	{
	public:
		...
		// IImGuiVRHelperInterface006 (client-owned physical panel surface).
		void SubmitClientSurface(uint32_t client_id,
			const ImGuiVRHelperPluginAPI::ClientSurfaceState* state) override;

		// IImGuiVRHelperInterface007 (client-owned scene-graph host).
		bool AcquirePanelTexture(uint32_t client_id,
			ImGuiVRHelperPluginAPI::PanelTextureHandle* out) override;
		void ReleasePanelTexture(ImGuiVRHelperPluginAPI::PanelTextureHandle* handle) override;
		void SetExternalPanelHost(uint32_t client_id, bool enabled) override;

		// IImGuiVRHelperInterface008 (focused-client physical host).
		void SubmitPhysicalHostSurface(uint32_t host_client_id,
			const ImGuiVRHelperPluginAPI::ClientSurfaceState* state) override;
		bool AcquireFocusedPanelTexture(uint32_t host_client_id,
			ImGuiVRHelperPluginAPI::PanelTextureHandle* out,
			uint32_t* out_content_client_id) override;
		void ReleaseHostedPanel(uint32_t host_client_id) override;
		void CycleHostedPanel(uint32_t host_client_id, int32_t direction) override;

		// IImGuiVRHelperInterface009 (physical-host client navigation).
		std::size_t GetHostedPanels(uint32_t host_client_id,
			ImGuiVRHelperPluginAPI::HostedPanelInfo* out, std::size_t capacity) override;
		bool SelectHostedPanel(uint32_t host_client_id, uint32_t content_client_id) override;
		void SetPhysicalHostInputPassthrough(uint32_t host_client_id, bool enabled) override;

		// IImGuiVRHelperInterface010 (host-scoped texture dimensions).
		bool SetPhysicalHostPanelSize(
			uint32_t host_client_id, uint32_t width, uint32_t height) override;
		...
		bool GetFocusedClientSurface(ImGuiVRHelperPluginAPI::ClientSurfaceState& out);
		bool IsClientExternallyHosted(uint32_t client_id);
	...
	private:
		...
		/// Mints the next ComboId returned by RegisterCombo. The ids are
		/// salted and permuted to avoid trivial enumeration.
		ImGuiVRHelperPluginAPI::ComboId NextComboIdLocked();
		uint32_t m_focused_client = 0;
		uint32_t m_physical_host_client = 0;
		bool m_has_physical_host_surface = false;
		bool m_physical_host_input_passthrough = false;
		ImGuiVRHelperPluginAPI::ClientSurfaceState m_physical_host_surface{};
		uint32_t m_physical_host_size_client = 0;
		uint32_t m_physical_host_panel_width = 0;
		uint32_t m_physical_host_panel_height = 0;
		...
		// Pixel size of a client texture. A focused physical host may override it;
		// otherwise the helper's configured base/HUD supersampling policy applies.
		...
		/// Replace the helper-owned overlay hit with the focused client's published
		/// physical surface hit.  This keeps every frame subsystem (drag, focus,
		/// input routing, callbacks, and cursor composition) on one pointer source.
		void ApplyFocusedClientSurfacePointer();
	...
	struct ClientRecord
	{
		...
		// R8G8B8A8_UNORM; dimensions follow the helper configuration unless an
		// active physical host requests an exact size for its focused panel.
		winrt::com_ptr<ID3D11Texture2D> texture;
		...
		bool hasClientSurface = false;
		ImGuiVRHelperPluginAPI::ClientSurfaceState clientSurface{};
