...
namespace ImGuiVRHelper::SettingsUI
{
	namespace
	{
		...
		// Visual quality / placement — first-class settings on the Overlay tab.
		void RenderDisplaySection(Overlay::Settings& s)
		{
			...
			ImGui::SliderInt("HUD supersample", &s.hudSupersample, 1,
				Overlay::Config::kMaxHUDSupersample, "%dx");
			int baseW, baseH;
		...
		void RenderWindow()
		{
			...
			// Clamp the window inside the current helper panel each frame.
	...
	bool Render(float dt)
	{
		...
		// helper panel texture this content renders into. Left alone, the
		// UI can fill a differently sized sub-rect based on the mirror window and
		// diverge from the wand cursor, whose normalized UV spans the full panel.
