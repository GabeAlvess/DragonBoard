...
void KeyHandler::RegisterSink()
{
    ...
    if (inputMgr) {
        ...
        logger::trace("KeyHandler sink registered successfully.");
    }
    else {
...
[[nodiscard]] KeyHandlerEvent KeyHandler::Register(uint32_t dxScanCode, KeyEventType eventType, KeyCallback callback)
{
    ...
    logger::trace("Registering callback with handle {} for key 0x{:X}, event type {}", handle, dxScanCode, (eventType == KeyEventType::KEY_DOWN ? "DOWN" : "UP"));
...
void KeyHandler::Unregister(KeyHandlerEvent handle)
{
    ...
    if (keyCallbacksIt != _registeredCallbacks.end()) {
        ...
        if (removedCount > 0) {
            logger::trace("Unregistered callback with handle {} for key 0x{:X}, event type {}", handle, info.key, (info.type == KeyEventType::KEY_DOWN ? "DOWN" : "UP"));
        }
        else {
