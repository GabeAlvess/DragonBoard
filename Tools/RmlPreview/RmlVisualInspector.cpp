...
namespace dragonboard::tools
{
    namespace
    {
        ...
        float ParsePixels(const std::string& value)
        {
            try {
    ...
    bool RmlVisualInspector::BeginMove(Rml::Element* element)
    {
        if (!element || element == _document) return false;
        const auto selector = BuildSelector(element);
        if (selector.empty()) return false;
        _movingElement = element;
        _movingSelector = selector;
        _movePrevious = _overrides;
        const auto found = _overrides.find(selector);
        const auto readValue = [&](const char* name) {
            if (found != _overrides.end()) {
                if (const auto property = found->second.find(name); property != found->second.end()) {
                    return ParsePixels(property->second);
                }
            }
            if (const auto* property = element->GetProperty(name)) return ParsePixels(property->ToString());
            return 0.0f;
        };
        _moveInitialLeft = readValue("left");
        _moveInitialTop = readValue("top");
        SelectElement(element);
        return true;
    }

    void RmlVisualInspector::UpdateMove(float deltaX, float deltaY)
    {
        if (!_movingElement || _movingSelector.empty()) return;
        const auto left = Rml::CreateString("%.0fpx", _moveInitialLeft + deltaX);
        const auto top = Rml::CreateString("%.0fpx", _moveInitialTop + deltaY);
        auto& properties = _overrides[_movingSelector];
        properties["position"] = "relative";
        properties["left"] = left;
        properties["top"] = top;
        _movingElement->SetProperty("position", "relative");
        _movingElement->SetProperty("left", left);
        _movingElement->SetProperty("top", top);
    }

    std::string RmlVisualInspector::EndMove()
    {
        if (!_movingElement || _movingSelector.empty()) return {};
        const auto selector = _movingSelector;
        _movingElement = nullptr;
        _movingSelector.clear();
        if (_overrides != _movePrevious) {
            _undo.push_back(std::move(_movePrevious));
            _redo.clear();
            UpdateButtons();
        }
        return selector;
    ...
    std::string RmlVisualInspector::SerializeOverrides() const
    {
    ...
    void RmlVisualInspector::SetOverrideSource(std::string_view source)
    {
        const Overrides previous = _overrides;
        _overrides.clear();
        std::istringstream stream{ std::string(source) };
        ParseOverrides(stream);
    ...
    void RmlVisualInspector::ParseOverrides(std::istream& stream)
    {
        std::string selector;
        std::string line;
